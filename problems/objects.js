...
const appleCount = (appleCountByName,name) => {
  return appleCountByName[name] //square brackets used bc name is a variable
};
...
const countOccurance = (str) => {
  let newOb = {}
  for(let i = 0; i < str.length; i++) {
    if(newOb[str[i].toLowerCase()] === undefined) {
      newOb[str[i].toLowerCase()] = 1
    }
    else {
    newOb[str[i].toLowerCase()] += 1
  }
